...
// Méthode pour déplacer le gardien selon la direction choisie
    ...
    public void action(Direction direction) {
        // Si l'entrepôt n'est pas terminé et que le gardien a une zone valide
        if (!entrepot.estTermine() && gardien.getZone() != null) {
            gardien.deplacer(direction); // Déplace le gardien dans la direction choisie
        }
    }
...
// Constructeur qui initialise l'entrepôt et le gardien
    ...
    public Controleur() {
        this.entrepot = new Entrepot(); // Crée une nouvelle instance de l'entrepôt
        this.gardien = new Gardien(); // Crée une nouvelle instance du gardien
    }
    ...
    public void chargerNiveau(int[][] niveau) {
        ...
        for (int i = 0; i < niveau.length; i++) {
            for (int j = 0; j < niveau[0].length; j++) {
                // Selon la valeur dans la matrice, on place un élément dans l'entrepôt
                switch (niveau[i][j]) {
                    case 1: 
                        entrepot.placerElement(i, j, TypeElement.MUR); // Place un mur
                        break;
                    case 2: 
                        entrepot.placerElement(i, j, TypeElement.CIBLE); // Place une cible
                        break;
                    case 3: 
                        entrepot.placerElement(i, j, TypeElement.CAISSE); // Place une caisse
                        break;
                    case 4: 
                        // Place le gardien à la position correspondante si la zone est libre
                        ...
                        break;
                    case 5: 
                        // Place à la fois une cible et une caisse
                        entrepot.placerElement(i, j, TypeElement.CIBLE);
                        entrepot.placerElement(i, j, TypeElement.CAISSE);
                        break;
                    case 6: 
                        // Place à la fois une cible et une caisse et positionne le gardien
...
// Vérifie si l'entrepôt est terminé (objectif atteint)
    ...
    public boolean estTermine() {
        return entrepot.estTermine(); // Retourne si l'entrepôt est terminé ou non
    }
...
// Réinitialise le niveau en rechargeant la matrice initiale
    ...
    public void reinitialiserNiveau() {
        if (niveauInitial != null) {
            chargerNiveau(niveauInitial); // Recharge le niveau initial
        }
    }
